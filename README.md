...
- 최근에는 Node.js + Supabase 기반 백엔드와, IoT 기기(MQTT)를 연동하는 서비스에 관심을 두고 있음
- 팀 프로젝트에서 일정 관리·갈등 조율 등 협업 경험을 다수 보유
...
### 팜므파탈 — 스마트팜 관리 플랫폼
> 기획 · 백엔드 · AI 모델 전담 (사실상 1인 풀스택)

Node.js + Supabase + MQTT 기반 홈가드닝/소규모 노지 관리용 스마트팜 플랫폼입니다. 룰엔진 기반 자동 제어, AI 식물 질병 진단, AI 챗봇, 일일 리포트, 위치 기반 날씨 API 연동 기능을 담당했습니다.

`Node.js` `Supabase` `MQTT` `Classification Model` `소셜 로그인`
...
### AjouOrder — 교내 카페 사이렌 오더 웹
> 기획부터 배포까지 전 과정 직접 진행

Node.js + React + Supabase로 만든 주문 관리 웹 서비스입니다. Toss 결제 API 연동, 주문 상태 관리, 예상 대기시간 기능을 구현했고, Render(백엔드) / Vercel(프론트) 배포까지 진행했습니다.
...
### Grove — 경력·활동 자산화 플랫폼
> 프론트엔드 리드 개발자 (웹 전체 담당) · Grove, 25-2 파란학기

주니어 대상 경력·활동 자산화 플랫폼으로, React 기반 프론트엔드 전체를 담당했습니다. 팀원 일정 관리와 갈등 조율, 백엔드와의 로그인 연동 협업을 경험했습니다.
...
롤러에 손이 접근하면 이를 감지해 경보를 출력하는 산재 예방 시스템입니다. MediaPipe와 3D 좌표 변환을 활용해 손의 위치를 추적했습니다.
...
- **전기차 충전소 안내 시스템** — DB 수업 프로젝트 (SQL/DB 설계)
...
**이수 과정**
- AI_Aigent 코드 리뷰 교육 수강

**자격증**
- SQLD (한국데이터산업진흥원, 2025.04)
- Google 애널리틱스 인증 GA4 (2025.06)
- ACP - Adobe Premiere Pro (Adobe, 2025.07)
